...
@Suite("XMPMetadata editable video-metadata fields")
struct XMPMetadataAccessorTests {
    ...
    func setOnEmptyDocumentRoundTrips() throws {
        ...
        let reparsed = try XMPMetadata(xml: metadata.xml)
    ...
    func setPreservesUnrelatedContent() throws {
        var metadata = try XMPMetadata(xml: sample(named: "sample3.xml"))
        metadata.scene = "Scene 4"
